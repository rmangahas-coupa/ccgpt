...
from config import AUTH, BASE_URL, MAX_RETRIES, LAST_RETRY_DELAY_MILLIS, MAX_RETRY_DELAY_MILLIS, JITTER_MULTIPLIER_RANGE
...
async def fetch_with_retry(url: str) -> dict:
    async for attempt in AsyncRetrying(
        ...
        wait=wait_exponential(multiplier=LAST_RETRY_DELAY_MILLIS / 1000, max=MAX_RETRY_DELAY_MILLIS / 1000)
    ):
        with attempt:
            async with aiohttp.ClientSession() as session:
                logger.info(f"Fetching URL: {url}")
                async with session.get(url, auth=aiohttp.BasicAuth(*AUTH), timeout=60) as response:
                    logger.info(f"Received response for URL: {url} with status: {response.status}")
                    if response.status == 429:
                        retry_delay_millis = int(response.headers.get('Retry-After', 0)) * 1000
...
async def get_all_page_ids(space_keys: List[str]) -> List[str]:
    ...
    for space_key in space_keys:
        ...
        while True:
            ...
            logger.info(f"Fetching page IDs for space key: {space_key} with start={start} and limit={limit}")
